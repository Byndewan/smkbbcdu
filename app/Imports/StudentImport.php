...
class StudentImport implements ToModel, WithBatchInserts, WithChunkReading, WithHeadingRow
{
    public function model(array $row)
    {
        ...
        if (!$newClass) {
            throw new Exception("Kelas Baru dengan kode '{$row['kode_kelas_baru']}' tidak ditemukan!");
        }
        if (!$major) {
            throw new Exception("Jurusan dengan kode '{$row['kode_jurusan']}' tidak ditemukan!");
        }
        if (!$year) {
            throw new Exception("Tahun Ajaran dengan kode '{$row['kode_tahun_ajaran']}' tidak ditemukan!");
        }
